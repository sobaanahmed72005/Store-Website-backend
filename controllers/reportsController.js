  ...
  try {
    ...
    if (period === '12m') {
      ...
      for (let i = 11; i >= 0; i--) {
        ...
        result.push({ bucket, revenue: Number(found?.revenue || 0), orders: Number(found?.orders || 0) });
      }
    ...
    logger.error('Error fetching revenue trend report:', err);
    res.status(500).json({ error: 'Failed to fetch revenue trend report' });
  }
}
...
export async function getRevenueSummary(req, res) {
  try {
    ...
    logger.error('Error fetching revenue summary report:', err);
    res.status(500).json({ error: 'Failed to fetch revenue summary report' });
  }
}
...
export async function getTopProducts(req, res) {
  try {
    const businessId = req.business.id;
    const limit = Number(clampLimit(req, 10));
    ...
    logger.error('Error fetching top products report:', err);
...
export async function getBottomProducts(req, res) {
  try {
    ...
    const [rows] = await pool.query(
      `SELECT p.id, p.name,
              COALESCE(SUM(CASE WHEN o.status NOT IN ('cancelled', 'returned') THEN oi.quantity ELSE 0 END), 0) AS totalQuantity,
              COALESCE(SUM(CASE WHEN o.status NOT IN ('cancelled', 'returned') THEN oi.quantity * oi.price ELSE 0 END), 0) AS totalRevenue
       FROM products p
       LEFT JOIN order_items oi ON (oi.product_ref = p.id OR oi.product_ref = p.slug)
    ...
    logger.error('Error fetching bottom products report:', err);
    res.status(500).json({ error: 'Failed to fetch bottom products report' });
  }
}
...
export async function getSalesByCity(req, res) {
  try {
    ...
    logger.error('Error fetching sales by city report:', err);
    res.status(500).json({ error: 'Failed to fetch sales by city report' });
  }
}
...
export async function getOrderStatusBreakdown(req, res) {
  try {
    ...
    res.json(rows.map((r) => numify(r, ['orders'])));
  } catch (err) {
    logger.error('Error fetching order status breakdown report:', err);
    res.status(500).json({ error: 'Failed to fetch order status breakdown report' });
  }
}
...
export async function getPaymentMethodBreakdown(req, res) {
  try {
    ...
    logger.error('Error fetching payment method breakdown report:', err);
    res.status(500).json({ error: 'Failed to fetch payment method breakdown report' });
  }
}
...
export async function getOrderValueHistogram(req, res) {
  try {
    ...
    res.json(HISTOGRAM_BUCKETS.map((bucket) => ({ bucket, orders: Number(byBucket.get(bucket) || 0) })));
  } catch (err) {
    logger.error('Error fetching order value histogram report:', err);
    res.status(500).json({ error: 'Failed to fetch order value histogram report' });
  }
}
...
export async function getSaleSplit(req, res) {
  try {
    ...
    logger.error('Error fetching sale split report:', err);
    res.status(500).json({ error: 'Failed to fetch sale split report' });
  }
}
